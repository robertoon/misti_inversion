...
def settling_velocity(phi):
    ...
    for atm_layer in range(ATMOSPHERE_LEVEL_STEP, UMBRELLA_HEIGHT + ATMOSPHERE_LEVEL_STEP, ATMOSPHERE_LEVEL_STEP):
        ...
        if v_layer == v_aux or elev <= TOPO_HEIGHT:
            return v_layer
        else:
            v_aux = v_layer
...
def tephra_load(x, y, disk_cell_x, disk_cell_y, mass, velocity):
    ...
    source_term = (velocity * mass) / (4 * pi * UMBRELLA_HEIGHT * DIFFUSION_COEF)
    denom = 4 * DIFFUSION_COEF * (UMBRELLA_HEIGHT/velocity)
    advection = (x - (disk_cell_x + U_WIND *
                      UMBRELLA_HEIGHT / velocity))**2 / denom
